...
use Hybrid\Cache\Contracts\LockProvider;
use Hybrid\Cache\Contracts\Repository as CacheContract;
use Hybrid\Cache\Contracts\Store;
use Hybrid\Cache\Events\CacheFlushed;
use Hybrid\Cache\Events\CacheFlushFailed;
use Hybrid\Cache\Events\CacheFlushing;
use Hybrid\Cache\Events\CacheHit;
use Hybrid\Cache\Events\CacheMissed;
use Hybrid\Cache\Events\ForgettingKey;
use Hybrid\Cache\Events\KeyForgetFailed;
use Hybrid\Cache\Events\KeyForgotten;
use Hybrid\Cache\Events\KeyWriteFailed;
use Hybrid\Cache\Events\KeyWritten;
use Hybrid\Cache\Events\RetrievingKey;
use Hybrid\Cache\Events\RetrievingManyKeys;
use Hybrid\Cache\Events\WritingKey;
use Hybrid\Cache\Events\WritingManyKeys;
use Hybrid\Cache\Limiters\ConcurrencyLimiterBuilder;
use Hybrid\Contracts\Events\Dispatcher;
use Hybrid\Tools\Carbon;
use Hybrid\Tools\Collection;
use Hybrid\Tools\InteractsWithTime;
use Hybrid\Tools\Traits\Macroable;
use InvalidArgumentException;
use function Hybrid\Tools\defer;
use function Hybrid\Tools\enum_value;

/**
 * @mixin \Hybrid\Cache\Contracts\Store
 */
class Repository implements ArrayAccess, CacheContract {
     ...
     *
     * @var \Hybrid\Cache\Contracts\Store
     */
    protected $store;
    ...
    public function flexible( $key, $ttl, $callback, $lock = null, $alwaysDefer = false ) {
        ...
        [
            $key                                        => $value,
            "hybrid_core:cache:flexible:created:{$key}" => $created,
        ] = $this->many( [ $key, "hybrid_core:cache:flexible:created:{$key}" ] );

        if ( in_array( null, [ $value, $created ], true ) ) {
            return tap( value( $callback ), fn( $value ) => $this->putMany( [
                $key => $value,
                "hybrid_core:cache:flexible:created:{$key}" => Carbon::now()->getTimestamp(),
            ], $ttl[1] ) );
        }
        ...
        $refresh = function () use ( $key, $ttl, $callback, $lock, $created ) {
            $this->store->lock(
                "hybrid_core:cache:flexible:lock:{$key}",
                $lock['seconds'] ?? 0,
                $lock['owner'] ?? null
            )->get( function () use ( $key, $callback, $created, $ttl ) {
                if ( $this->get( "hybrid_core:cache:flexible:created:{$key}" ) !== $created ) {
                    return;
                }
        ...
        defer( $refresh, "hybrid_core:cache:flexible:{$key}", $alwaysDefer );
     ...
     * @return TReturn
     *
     * @throws \Hybrid\Cache\Contracts\LockTimeoutException
     */
     ...
     *
     * @return \Hybrid\Cache\Limiters\ConcurrencyLimiterBuilder
     */
    ...
    protected function getSeconds( $ttl ) {
        ...
        return (int) ( 0 < $duration ? $duration : 0 );
    }
     ...
     *
     * @return \Hybrid\Cache\Contracts\Store
     */
