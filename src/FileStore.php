...
use Hybrid\Filesystem\LockableFile;
...
class FileStore implements LockProvider, Store {
    ...
    /**
     * The Hybrid Filesystem instance.
     *
    ...
    public function forget( $key ) {
        if ( $this->files->exists( $file = $this->path( $key ) ) ) {
            return tap( $this->files->delete( $file ), function ( $forgotten ) use ( $key ) {
                if (
                    $forgotten
                    && $this->files->exists( $file = $this->path( "hybrid_core:cache:flexible:created:{$key}" ) )
                ) {
    ...
    protected function expiration( $seconds ) {
        ...
        return 0 === $seconds || 9999999999 < $time ? 9999999999 : $time;
    }
