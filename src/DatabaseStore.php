...
use Hybrid\Database\ConnectionInterface;
use Hybrid\Database\PostgresConnection;
use Hybrid\Database\QueryException;
use Hybrid\Database\SQLiteConnection;
use Hybrid\Database\SqlServerConnection;
...
class DatabaseStore implements LockProvider, Store {
     ...
     *
     * @var \Hybrid\Database\ConnectionInterface
     */
     ...
     *
     * @param \Hybrid\Database\ConnectionInterface $connection
     * @param string                               $table
     * @param string                               $prefix
    ...
    protected function forgetMany( array $keys ) {
        $this->table()->whereIn( 'key', ( new Collection( $keys ) )->flatMap( fn( $key ) => [
            $this->prefix . $key,
            "{$this->prefix}hybrid_core:cache:flexible:created:{$key}",
        ] )->all() )->delete();
    ...
    protected function forgetManyIfExpired( array $keys, bool $prefixed = false ) {
        $this->table()
            ->whereIn( 'key', ( new Collection( $keys ) )->flatMap( fn( $key ) => $prefixed ? [
                $key,
                $this->prefix . 'hybrid_core:cache:flexible:created:' . Str::chopStart( $key, $this->prefix ),
            ] : [
                "{$this->prefix}{$key}",
                "{$this->prefix}hybrid_core:cache:flexible:created:{$key}",
            ] )->all() )
     ...
     *
     * @return \Hybrid\Database\Query\Builder
     */
     ...
     *
     * @return \Hybrid\Database\ConnectionInterface
     */
    ...
    protected function unserialize( $value ) {
        ...
        if ( null !== $this->serializableClasses ) {
