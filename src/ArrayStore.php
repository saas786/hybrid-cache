...
class ArrayStore extends TaggableStore implements LockProvider {
     ...
     * The array of locks.
     *
     * @var array<string, array{owner: ?string, expiresAt: ?\Hybrid\Tools\Carbon}>
     */
    public $locks = [];
    ...
    public function all( $unserialize = true ) {
        if ( false === $unserialize || false === $this->serializesValues ) {
            return $this->storage;
        }
    ...
    public function get( $key ) {
        ...
        if ( 0 !== $expiresAt && ( Carbon::now()->getPreciseTimestamp( 3 ) / 1000 ) >= $expiresAt ) {
            $this->forget( $key );
    ...
    protected function toTimestamp( $seconds ) {
        return 0 < $seconds ? ( Carbon::now()->getPreciseTimestamp( 3 ) / 1000 ) + $seconds : 0;
    }
